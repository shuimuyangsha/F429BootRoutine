...
/**
  ...
  * @retval 无
  */
void WM8978_I2C_GPIO_Config(void)
{
...
/**
  ...
  * @retval 无
  */
void WM8978_I2C_Mode_Configu(void)
{
...
//  I2C_SendData(WM8978_I2C, RegisterAddr);
...
//  I2C_SendData(WM8978_I2C, RegisterValue);
...
uint8_t wm8978_Init(void)
{
	uint8_t res;
/*
 * I2C总线与触摸屏芯片共用，已在bsp_init函数中初始化及配置完成，
 * 在此不需要初始化及配置。
 */
//	WM8978_I2C_GPIO_Config();    
//	WM8978_I2C_Mode_Configu();
...
void I2S_Stop(void)
{
	...
	/* 关闭 I2S2 APB1 时钟 */
	RCC_APB1PeriphClockCmd(WM8978_CLK, DISABLE);
}
...
void I2Sxext_Mode_Config(const uint16_t _usStandard, const uint16_t _usWordLen,const uint32_t _usAudioFreq)
{
	...
	/* I2S2 外设配置 */
	I2Sext_InitStructure.I2S_Mode = I2S_Mode_MasterRx^(1<<8);			/* 配置I2S工作模式 */
...
{  
  ...
  RCC_AHB1PeriphClockCmd(I2Sx_DMA_CLK,ENABLE);//DMA1时钟使能 
	...
	DMA_ClearITPendingBit(I2Sxext_RX_DMA_STREAM,DMA_IT_FEIF4|DMA_IT_DMEIF4|DMA_IT_TEIF4|DMA_IT_HTIF4|DMA_IT_TCIF4);//清空DMA1_Stream4上所有中断标志
