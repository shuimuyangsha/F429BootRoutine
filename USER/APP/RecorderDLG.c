...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateRecorder[] = {
  { FRAMEWIN_CreateIndirect, "Recorder",   0,              0,   0,   800, 480, 0, 0x0,    0 },
  { RADIO_CreateIndirect,    "Input",      GUI_ID_RADIO0,  40,  60,  100, 50,  0, 0x1e02, 0 },
  { CHECKBOX_CreateIndirect, "EAROutput",  GUI_ID_CHECK0,  40,  160, 100, 20,  0, 0x0,    0 },
  { RADIO_CreateIndirect,    "AudioFreq",  GUI_ID_RADIO1,  170, 60,  100, 175, 0, 0x1e06, 0 },    
  { SLIDER_CreateIndirect,   "gain",       GUI_ID_SLIDER2, 320, 90,  30,  150, 8, 0x0,    0 }, 
  { TEXT_CreateIndirect,     "Gaindata",   GUI_ID_TEXT3,   305, 60,  60,  25,  0, 0x64,   0 },  
  { BUTTON_CreateIndirect,   "start",      GUI_ID_BUTTON0, 150, 370, 80,  40,  0, 0x0,    0 },
  { TEXT_CreateIndirect,     "recname",    GUI_ID_TEXT4,   100, 280, 200, 35,  0, 0x0,    0 },
  { TEXT_CreateIndirect,     "time",       GUI_ID_TEXT5,   100, 320, 60,  25,  0, 0x64,   0 },   
  
  { TEXT_CreateIndirect,     "playname",   GUI_ID_TEXT6,   470, 150, 230, 35,  0, 0x0,    0 }, 
  { SLIDER_CreateIndirect,   "Schedule",   GUI_ID_SLIDER1, 470, 230, 230, 30,  0, 0x0,    0 }, 
  { TEXT_CreateIndirect,     "playtime",   GUI_ID_TEXT1,   470, 260, 40,  20,  0, 0x64,   0 },  
  { TEXT_CreateIndirect,     "alltime",    GUI_ID_TEXT2,   660, 260, 40,  20,  0, 0x64,   0 },   
  { SLIDER_CreateIndirect,   "Volume",     GUI_ID_SLIDER0, 470, 310, 200, 30,  0, 0x0,    0 },
  { TEXT_CreateIndirect,     "volumedata", GUI_ID_TEXT0,   675, 315, 30,  20,  0, 0x64,   0 }, 
  { BUTTON_CreateIndirect,   "replay",     GUI_ID_BUTTON1, 540, 370, 80,  40,  0, 0x0,    0 }, 
  { BUTTON_CreateIndirect,   "back",       GUI_ID_BUTTON2, 470, 375, 30,  30,  0, 0x0,    0 }, 
  { BUTTON_CreateIndirect,   "next",       GUI_ID_BUTTON3, 660, 375, 30,  30,  0, 0x0,    0 }, 
  
  { TEXT_CreateIndirect,     "state",      GUI_ID_TEXT7,   470, 40,  230, 30,  0, 0x64,   0 }, 
...
{ 
  ...
  while(1)
  {
    while(Isread_Rec==0)
    {
      ...
      if((f_result!=FR_OK)||(Dev_Rec.ucStatus!=STA_RECORDING)||(Flag_ICON2==0))
      {        
        ...
        f_result=f_close(&f_file);
        sprintf(tempstr,"%s%s...",UTF8_IDLE,UTF8_STATE);
        if(Flag_ICON2)TEXT_SetText(WM_GetDialogItem(hWin, GUI_ID_TEXT7),tempstr);
        I2S_Stop();		/* 停止I2S录音和放音 */
			  wm8978_Reset();	/* 复位WM8978到复位状态 */
        return;
      }
...
static void replay(const char *recfile,WM_HWIN hWin)
{
  ...
  while(1)
  {
    while((Isread_Rec==0)&&(Dev_Rec.ucStatus==STA_REPLAYING)&&Flag_ICON2)
    {
      if(timecount>=2)      
      { 
    ...
    Isread_Rec=0;
    if((f_result!=FR_OK)||(f_file.fptr==f_file.fsize)||(Dev_Rec.ucStatus!=STA_REPLAYING)||(Flag_ICON2==0))
		{
      printf("播放完或者读取出错退出...\r\n");
      I2S_Play_Stop();
      f_result=FR_OK;
...
static void _cbDialogRecorder(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    ...
    GUI_DrawRect(10,10,390,430);
    GUI_DrawRect(430,10,740,90);
    GUI_DrawRect(430,120,740,430);
    GUI_SetColor(GUI_LIGHTBLUE);
    GUI_DispStringAt(UTF8_RECORDE,20,0);
    GUI_DispStringAt(UTF8_STATE,440,0);  
    GUI_DispStringAt(UTF8_REPLAY,440,110);  
  
    GUI_SetColor(GUI_LIGHTGREEN);  
    GUI_SetFont(&XBF_XINSONGTI19);
    GUI_DispStringAt(UTF8_INPUT,40,35);
    GUI_DispStringAt(UTF8_FREQ,170,35);
    GUI_DispStringAt(UTF8_GAIN,310,35);  
    GUI_DispStringAt(UTF8_EAR,40,140);
    GUI_DispStringAt(UTF8_NAME,50,287);
    GUI_DispStringAt(UTF8_SCHEDULE,50,322);  
    GUI_DispStringAt(UTF8_SCHEDULE,470,210);
    GUI_DispStringAt(UTF8_VOLUME,470,290);
    break;
  // USER END
  default:
    WM_DefaultProc(pMsg);
...
void FUN_ICON2Clicked(void) 
{
  ...
  hWin = GUI_CreateDialogBox(_aDialogCreateRecorder, GUI_COUNTOF(_aDialogCreateRecorder), _cbDialogRecorder, HDTWIN, 0, 0);
